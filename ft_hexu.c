...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/10 18:08:20 by adaly             #+#    #+#             */
/*   Updated: 2017/06/10 18:11:00 by adaly            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void		ft_hexu_width(t_pfconv *current)
{
	char		*width;
	uintmax_t	size;
...
static void		ft_hexu_precision(t_pfconv *current)
{
	char		*precistr;
	uintmax_t	size;
...
static void		ft_hexu_mod(t_pfconv *current)
{
	ft_hexu_width(current);
	ft_hexu_precision(current);
	current->prefix = NULL;
	if (current->flags[4])
		current->prefix = ft_strdup("0");
}

static void		ft_hexu_finalize(t_pfconv *current)
{
	char		*final;
	uintmax_t	where;
...
void			ft_hexu(t_pfconv *current, va_list args)
{
	uintmax_t	valsize;
	uintmax_t	*val;
	char		*str;
