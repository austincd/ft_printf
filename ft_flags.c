...
/*   Updated: 2017/06/06 21:18:16 by adaly            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_hash(t_pfconv *current)
{
	if (current->flags[4])
	{
		...
		if (current->type == 'X' && ft_atoi(current->string))
			ft_restrcat(&(current->prefix), "0X");
		if (current->widthstr)
			current->widthstr[ft_strlen(current->widthstr) - ft_strlen(current->prefix)] = 0;
	}
}
		...
		current->widthstr && !current->flags[0])
		{
			...
			if (current->precision < current->width && current->precision > 0)
			{
				if (ft_strchr(current->string, '-'))
					ft_memset(current->widthstr, ' ', (current->width - (current->precision + 1)));
				else
					ft_memset(current->widthstr, ' ', current->width - current->precision);
			}
		}
	}
	ft_strdel_plus(&ints);
...
void	ft_minus(t_pfconv *current)
{
	if (current)
	{
		...
		{
//			temp = current->string;
//			current->string = current->widthstr;
//			current->widthstr = temp;
		}
	}
}
