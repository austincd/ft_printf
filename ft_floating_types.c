...
/*   Updated: 2017/06/04 03:15:09 by adaly            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_g(t_pfconv *current, long double num)
{
	...
	if (length1 + ft_strlen(current->exponent) < length2)
		free(temp);
	else
	{
		free(current->string);
...
int	ft_floating_types(t_pfconv *current, va_list args)
{
	...
	if (current->precision <= 0 && ft_lowercase(current->type) != 'a' && ft_lowercase(current->type) != 'g')
		current->precision = 6;
