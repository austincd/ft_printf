...
/*   Updated: 2017/06/09 23:12:20 by adaly            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_finalize_wstr(t_pfconv *current)
{
	...
	if (current)
	{
		...
		{
			final = (wchar_t*)ft_memalloc(sizeof(wchar_t) * (lens[0] + lens[1]));
			if (final)
			{
				ft_memcpy(final, current->widthstr, (sizeof(wchar_t) * lens[0]));
				ft_memcpy(final + lens[0], current->string, (sizeof(wchar_t) * lens[1]));
			}
